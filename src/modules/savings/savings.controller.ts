...
export class SavingsController {
 ...
 @UseGuards(AuthJwtAuthGuard)
 @Post('add')
 async addMoney(@Body('amount') amount: number, @Req() req) {
  const userId = req.user;
   return await this.savingsService.addMoney(amount, userId);
 }

 // Endpoint to deduct money from the balance
 @UseGuards(AuthJwtAuthGuard)
 @Post('deduct')
 async deductMoney(@Body('amount') amount: number, @Req() req) {
  const userId = req.user;
   return await this.savingsService.deductMoney(amount, userId);
 }
}
