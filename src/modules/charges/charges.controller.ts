...
export class ChargesController {
  ...
  @Roles(Role.SuperAdmin, Role.Admin, Role.Cassier)
  @Get("spa-report")
  getSpaRevenus(@Req() req: ReqOptions) {
    let query = req.query.s ? JSON.parse(req.query.s as string) : {};
    if (!query.filter) query.filter = {};
    return this.chargesService.getSpaRevenus(query);
  }
  @UseGuards(AuthJwtAuthGuard, RolesGuard)
  @Roles(Role.SuperAdmin, Role.Admin, Role.Cassier)
  @Get("charges-report")
  getChargesReport(@Req() req: ReqOptions) {
    let query = req.query.s ? JSON.parse(req.query.s as string) : {};
    if (!query.filter) query.filter = {};
    return this.chargesService.getChargesReport(query);
  }
  @UseGuards(AuthJwtAuthGuard, RolesGuard)
  @Roles(Role.SuperAdmin, Role.Admin, Role.Cassier)
  @Get("payments-report")
  ...
  getProgressStats(@Req() req: ReqOptions) {
    let query = req.query.s ? JSON.parse(req.query.s as string) : {};
    if (!query.filter) query.filter = {};
    return this.chargesService.getProgressStats(query);
  }
  @UseGuards(AuthJwtAuthGuard, RolesGuard)
  @Roles(Role.SuperAdmin, Role.Admin, Role.Cassier)
  @Get("top-stats")
